...
class TheThumbnail(Scene):
   def construct(self):
      ...
      yay1 = Square().shift(UP, LEFT)
      yay2 = Square().shift(DOWN, LEFT).set_color(BLUE)
      yay3 = Square().shift(DOWN, RIGHT)
      yay4 = Square().shift(UP, RIGHT).set_color(RED)

      square1 = Square().scale(0.5).shift(2*DOWN, LEFT).set_color(BLUE)
      rectangle1 = Rectangle(height=3, width=1).shift(LEFT)
      square2 = Square().scale(1.5).shift(RIGHT).set_color(RED)
      rectangle2 = Rectangle(height=1, width=3).shift(RIGHT, 2*DOWN)
      firstgroupyay = VGroup(square1, rectangle1, square2, rectangle2).scale(0.25).shift(RIGHT*5.75, 1.25*UP)



      yaygroup1 = VGroup(yay1, yay2, yay3, yay4).scale(0.25).shift(LEFT*6.1, 0.75*UP)

      thumbnailtitle = TextMobject("The Hardy-Weinberg Theorem:").scale(1.4).shift(UP)

      thumbnaimsub = TextMobject("A Biological Thought Experiment").next_to(thumbnailtitle, DOWN/2).scale(1.4)

      ellipse = Ellipse().shift(DOWN*1.4).scale(1.3)
      inell = TextMobject("Population").set_color(YELLOW).shift(DOWN*1.4).scale(0.8)
      evol = TextMobject("Migration").set_color(PURPLE).shift(DOWN*1.4, RIGHT*4.6).scale(0.8)
      mut = TextMobject("Mutation").set_color(PURPLE).shift(DOWN*1.4, LEFT*4.6).scale(0.8)
      arrow1 = Arrow(evol.get_left(), ellipse.get_right()).scale(0.9)
      arrow2 = Arrow(mut.get_right(), ellipse.get_left()).scale(0.9)

      Group1 = VGroup(ellipse, inell, evol, mut, arrow1, arrow2)


      self.add(thumbnailtitle, finalpos, thumbnaimsub, Group1, firstgroupyay, yaygroup1)
